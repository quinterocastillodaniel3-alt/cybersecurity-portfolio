...
*   **[Python Conditionals Lab](./python-conditionals-lab):** Implementing access control and logic evaluations using `if`, `elif`, and logical operators (`and`/`or`).
*   **[Python Loops Lab](./python-loops-lab):** Automating repetitive security measures and iterating over network data using `for` and `while` loops.
*   **[Python Security Algorithms](./python-security-algorithms):** Developing step-by-step algorithms with nested logic to authenticate users and verify assigned hardware devices.

### ⚙️ Functions & Data Manipulation
*   **[Python Custom Functions](./python-custom-functions):** Defining reusable Python functions to format data and generate standardized security alerts.
*   **[Python Log Analysis Functions](./python-log-analysis-functions):** Using built-in functions (`max()`, `sorted()`) and custom functions to parse failed login attempts and calculate threat metrics.
*   **[Python String Manipulation](./python-string-manipulation):** Slicing and extracting specific data points, such as device IDs and URL domains, from structured text data.
